...
function TradeskillInfo:OnInitialize()
	local dbDefaults = {
		profile = {
			...
			ColorUsedInAmountAccount = { r=1.0, g=1.0, b=1.0 },
			ColorUsedInAmountCharacter = { r=1.0, g=1.0, b=1.0 },
...
function TradeskillInfo:GetCombineComponents(id, getVendorPrice, getAuctioneerPrice)
	...
	-- finds each substring separated by any space characters
...
local function AddTable(tbl1,tbl2)
	for i,v in pairs(tbl2) do
		if tbl1[i] then
			for key,value in pairs(tbl2[i]) do
				tbl1[i][key] = tbl1[i][key] + value
			end
			--tbl1[i] = tbl1[i] + v;
		else
			tbl1[i] = v;
...
function TradeskillInfo:ShowingTooltipUsedInAmountAccount()
	return self.db.profile.TooltipUsedInAmountAccount;
end
